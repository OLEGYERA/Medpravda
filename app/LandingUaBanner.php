...
class LandingUaBanner extends Model
{
    protected $fillable = ['title', 'description', 'imgalt', 'imgtitle', 'button'];
    ...
    public function updateBackground($request)
    {
        ...
        if ($request->hasFile('background.image')) {
            $path = $this->uploadBgImage($request->file('background.image'), 'banners/backgrounds');
            if ($path) {
