        ...
        this.update = function () {
            ...
            if (rng.isCollapsed() && rng.isOnAnchor()) {
                ...
                if(href.indexOf('-term') !== -1){
                    this.$popover.find('a').attr('href', document.location.origin + '/manage/dependency/term#/edit/' + href.replace(/#/g, '')).html(href + ' - <b>термин</b>');
                    $(this.$popover[0]).children('.popover-content').children('.note-btn-group').css('display','none')
                }else{
                    this.$popover.find('a').attr('href', href).html(href);
                    $(this.$popover[0]).children('.popover-content').children('.note-btn-group').css('display','inline-block')
                }
