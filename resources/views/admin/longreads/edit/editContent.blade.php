...
@include('admin.longreads.aside', ['id' => $longread->id, 'lang' => 'ru'])

{!! Form::open(['url'=>route('longread_edit_content', ['id' => $longread->id, 'lang' => 'ru']), 'method'=>'POST', 'class'=>'form-horizontal', 'files'=>true]) !!}
<div class="lg">
    ...
    <div class="template-field edit">
        @foreach($longread_template->parts()->orderBy('priority', 'asc')->get() as $item)
            @php($build = $item->build()->where('longreads_id', $longread->id)->where('lang', 'ru')->first())
            @switch($item->type)
                @case('tir')
                    <h2>#{{$item->priority}}</h2>
