...
@include('admin.longreads.aside')

{!! Form::open(['url'=>route('longread_create_main'), 'method'=>'POST', 'class'=>'form-horizontal']) !!}


<div class="wrap__block lg">
    <h2>Лонгрид Ru</h2>
    <div class="input__group">
        {!! Form::text('title', old('title') ? : '' , ['placeholder'=>'Заголовок страницы', 'id'=>'title', 'class'=>'form-control ru-title']) !!}
    ...
    <div class="input__group info-check label-stay">
        <input type="checkbox" {{ old('confirmed_ru') ? 'checked' : ''}} value="1" name="confirmed_ru" id="confirmed_ru">
...
<div class="wrap__block lg">
    <h2>Лонгрид Ua</h2>
    <div class="input__group">
        {!! Form::text('utitle', old('utitle') ? : '' , ['placeholder'=>'Заголовок сторінки', 'id'=>'utitle', 'class'=>'form-control']) !!}
        {{ Form::label('utitle', 'Заголовок сторінки') }}
    </div>

    <div class="input__group">
        {!! Form::text('udescription', old('udescription') ? : '' ,
    ...
    <div class="input__group info-check label-stay">
        <input type="checkbox" {{ old('confirmed_ua') ? 'checked' : ''}} value="1" name="confirmed_ua" id="confirmed_ua">
...
<div class="wrap__block lg">
    ...
    <div class="input__group">
        {!! Form::text('priority',  old('priority') ? : 0 ,
                 ['placeholder'=>'priority', 'id'=>'priority', 'class'=>'form-control']) !!}
        {{ Form::label('priority', 'Приоритет(0-255)') }}
    </div>
    <div class="input__group">
        <label for="template_id" style="position: static; margin-bottom: 10px; display: block">
            Выберите шаблон
        </label>
        <select value="{{ old('template_id') ? : null}}" id="template_id" name="template_id" style="width: 100%; margin-bottom: 10px;" placeholder = 'Выберите шаблон'>
            @foreach($longread_templates as $longread_template)
                @if($longread_template->id == old('template_id'))
