<section class="content page-articles m-top">
    <div class="wrap blue-circle">
        ...
        {{--BreadCrumbs--}}
        <h1>{{ $cat->utitle ?? ($tag->uname ?? '') }}</h1>
    </div>
    @if(!empty($articles))
        <div class="big-news margin-minus all-big-news">
            <article class="news">
                <a href="{{ route('ua_articles', ['article_alias'=>$articles[0]->alias]) }}/">
                    <div class="article-img">
                        @if(!empty($articles[0]->image->path))
                            <img src="{{ asset('asset').'/images/articles/ua/main/'.$articles[0]->image->path }}"
                                 alt="{{ $articles[0]->image->alt }}" title="{{ $articles[0]->image->title }}"
                                {!! image_width_height_return_tags(asset('asset').'/images/articles/ua/main/'.$articles[0]->image->path) !!}
                            >
                        @else
                        ...
                        <div class="date-link">
                            <div class="article-date">
                                {{ $articles[0]->created_at->format('d')
                                        . ' '  . trans('ua.'.$articles[0]->created_at->format('m'))
