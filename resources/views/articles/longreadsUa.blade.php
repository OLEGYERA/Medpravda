<section class="content page-articles m-top">
    <div class="wrap blue-circle">
        ...
        <div class="bread-crumbs breadcrumbs mobile-display-none" id="breadcrumbs" itemscope
            ...
            <div itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="button">
                <span itemprop="name" class="label1">Лонгріди</span>
    ...
    @if(!empty($longreads))
        <div class="big-news margin-minus all-big-news">
            <article class="news">
                <a href="{{ route('longread_show_ua', ['alias'=>$longreads[0]->alias]) }}/">
                    <div class="article-img">
                        @if(!empty($longreads[0]->image()->first()))
                            <img src="{{ $longreads[0]->image()->first()->upath }}"
                                 alt="{{ $longreads[0]->image()->first()->ualt }}" title="{{ $longreads[0]->image()->first()->utitle }}"
                                    {!! image_width_height_return_tags($longreads[0]->image()->first()->upath) !!}
                            >
                        @else
                    ...
                    <div class="article-info">
                        <div class="article-title">{{ $longreads[0]->utitle }}</div>
                        <p class="article-category">Лонгріди</p>
                        <div class="article-text">{!! str_limit(strip_tags($longreads[0]->udescription), 156) !!}</div>
                        <div class="date-link">
        ...
        <div class="two-column-articles article-wrap section-interest-art">
            <div class="two-big-news">
                @foreach($longreads as $key => $longread)
                    @continue($key == 0)
                    <article class="news">
                        <a href="{{ route('longread_show_ua', ['alias'=>$longread->alias]) }}/">
                            <div class="article-img">
