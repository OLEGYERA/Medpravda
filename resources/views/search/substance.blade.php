<section class="content m-top">
    <div class="wrap">
        ...
        <h1>Сортировка по: {{ !empty($substance->title) ? 'Действующему веществу - '.$substance->title : 'Действующему веществу' }}</h1>

        @include('search.nav')
    </div>
    <div class="section-title-meta-icon serch-height">
        <h3>Сортировка препаратов по действующему веществу:
            @if(!empty($letter))
                <a>{{ $letter }}</a>
            @endif
        </h3>
