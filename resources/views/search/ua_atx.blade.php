<section class="content m-top">
    <div class="wrap">
        ...
        <div class="bread-crumbs breadcrumbs mobile-display-none" id="breadcrumbs" itemscope
            ...
            @empty($atx)
                <div itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="button">
                    <span itemprop="name" class="label1">Сортування за АТX-класифікацією</span>
            ...
            @if(!empty($atx->parents))
                <div itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="button">
                    <a href="{{ route('search_atx_u') }}/">
                        <span class="label1">Сортування за АТX-класифікацією</span>
                    </a>
                    <meta itemprop="position" content="2"/>
                </div>
                <div itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="button">
                    <a href="{{ route('search_atx_u', ['val'=>$atx->parents->class ]) }}/" itemprop="item">
            ...
            @elseif(!empty($atx))
                <div itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="button">
                    <a href="{{ route('search_atx_u') }}/" itemprop="item">
                        <span class="label1" itemprop="name">Сортування за АТX-класифікацією</span>
                    </a>
                    <meta itemprop="position" content="2"/>
                </div>
                <div itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="button">
        ...
        @if(!empty($letter))
            <h1>Сортування за: ATX - {{!empty($atx) ? $atx->class .' : '. $atx->name : $letter}}</h1>
        @else
            <h1>Сортування за: ATX </h1>
        @endif
    ...
    <div class="section-title-meta-icon serch-height">
        <h3>Сортування за АТX-класифікацією:
            @if(!empty($letter))
                <a>{{ $letter }}</a>
            @endif
        </h3>
