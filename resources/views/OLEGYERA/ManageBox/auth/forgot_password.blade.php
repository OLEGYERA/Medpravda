...
<head>
    ...
    <title>ВОССТАНОВИТЬ ПАРОЛЬ ОТ МЕДПРАВДЫ</title>
...
<div class="mp-background flex-row-c-c">
    <div class="mp-visual-box flex-row-sb-s">
        ...
        <form action="{{route('manage.forgot.password')}}" method="POST" class="mp-auth-box flex-col-c-c">
            {{ csrf_field()}}
            <h1 class="auth-title">
                ВОССТАНОВИТЬ ПАРОЛЬ
            </h1>
            ...
            <button class="btn-auth">ОТПРАВИТЬ НОВЫЙ ПАРОЛЬ</button>
            <div class="forgot-row">Забыли <span class="textmark"><a href="{{route('manage.forgot.login')}}">логин</a>?</span> / Страница <span class="textmark"><a href="{{route('manage.login')}}">входа</a></span></div>
        </form>
    </div>
</div>
</body>
