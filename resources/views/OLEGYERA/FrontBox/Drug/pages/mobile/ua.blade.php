@include('OLEGYERA.FrontBox.Drug.modules.mobile-menu.ua')
...
<script type="application/ld+json">
{
  ...
  "url": "{{ route('m.ua.drug', ['alias' => $drug->alias])}}",
...
<div class="full-content"
    ...
    <link itemprop="mainEntityOfPage" href="{{route('m.ua.drug', ['alias' => $drug->alias])}}" />
    <section class="page-content mp-anchor" id="startPage">
                ...
                <div class="author-info">
                    ...
                    @if($drug->creator->editor)
                        <div class="author-links">
                            ...
                            @endif
                            <link itemprop="sameAs" href="{{route('m.ua.editors')}}/#{{$drug->creator->id}}" />
                            <a target="_blank" href="{{route('m.ua.editors')}}/#{{$drug->creator->id}}">{{$drug->creator->surname}} {{$drug->creator->name}}</a>
                        </div>
                    @endif
                </div>
                @if($drug->creator->editor)
        ...
        <div class="row-drug-presentation">
            @if($drug->setting->recipe)
                <a href="{{route('m.ua.prescription_conditions')}}" target="_blank" class="recipe-alert">
                    <span class="glyph other"></span>
                    <span class="under-title">Інформація про використання рецептурних препаратів</span>
                </a>
            @endif
            <h1 class="drug-title" itemprop="headline name">{{$drug->utitle}}
            ...
            <div class="page-row">
                <div class="el-img">
                    <figure class="avatar">
                        <img src="{{asset('/gallery/' . getCategoryName($drug->image->category_id) . '/medium/' . $drug->image->path)}}" alt="{{$drug->title}} фото, інструкція">
                    </figure>
            ...
            <div class="drug-intro-dependencies">
                ...
                @if($drug->dependency->fabricator)
                    <div class="mp-row-present">
                        ...
                        <div class="info-row">
                            <a href="{{route('m.ua.catalog.list.fabricator', ['val' => $drug->dependency->fabricator->alias])}}" target="_blank" class="link-blank"><span class="glyph blank_link"></span></a>
                            <a href="{{route('m.ua.catalog.list.fabricator', ['val' => $drug->dependency->fabricator->alias])}}" class="external-link">{{$drug->dependency->fabricator->utitle}}@if($drug->dependency->fabricator_location), {{$drug->dependency->fabricator_location->utitle}}@endif</a>
                        </div>
                    </div>
                @endif
                @if($drug->dependency->pharma)
                    <div class="mp-row-present">
                        <h3 class="title-row">Фарм. група:</h3>
                        <div class="info-row">
                            <a href="{{route('m.ua.catalog.drug.pharma', ['val' => $drug->dependency->pharma->alias])}}" target="_blank" class="link-blank"><span class="glyph blank_link"></span></a>
                            <a href="{{route('m.ua.catalog.drug.pharma', ['val' => $drug->dependency->pharma->alias])}}" class="external-link">{{$drug->dependency->pharma->utitle}}</a>
                        </div>
                    </div>
                @endif
            </div>
    ...
    @include('OLEGYERA.FrontBox.MODULES.asides.mobile.ua', ['data' => $drug, 'is_mobile' => $is_mobile])
</div>
