        ...
        @if($structure_settings->bgTop)
            <div class="article-preloader" style="background-image: url({{asset('/gallery/' . getCategoryName($article->dependency->image->category_id) . '/large/' . $article->dependency->image->path)}});">
                <div class="preloader-content">
                    ...
                    <h1 class="article-title">{{$article->utitle}}</h1>
                </div>
            </div>
        @endif
            ...
            <div class="article-content @if($structure_settings->fullWidth) full @endif">
                <div class="row-author">
                        ...
                        <div class="author-info">
                            ...
                            @if($article->dependency->creator->editor)
                                <div class="author-links">
                                    ...
                                    @endif
                                    <link itemprop="sameAs" href="{{route('ua.editors')}}/#{{$article->dependency->creator->id}}" />
                                    <a target="_blank" href="{{route('ua.editors')}}/#{{$article->dependency->creator->id}}">{{$article->dependency->creator->surname}} {{$article->dependency->creator->name}}</a>
                                </div>
                            @endif
                        </div>
                        @if($article->dependency->creator->editor)
                    ...
                    <time datetime="{{$article->created_at->format('Y-m-d H:i:s')}}" class="last-update">
                        Дата оновлення: {{$article->created_at->format('d')}} {{renderNameOfMonth($article->created_at->format('M'), 'ua')}} {{$article->created_at->format('Y')}}
                    </time>
                </div>
                @if(!$structure_settings->bgTop)
                    <div class="row-article-presentation">
                        ...
                        <div class="article-img">
                            <figure class="avatar">
                                <img src="{{asset('/gallery/' . getCategoryName($article->dependency->image->category_id) . '/large/' . $article->dependency->image->path)}}" alt="{{$article->utitle}} фото, инструкция">
                            </figure>
                ...
                @if($structure_settings->float)
                    <div class="article-blocks">
                        @foreach($article->dependency->structure->blocks as $block)
                            ...
                            @if($block_article === null && $block_article->ua === null && $block_article->ua === '')
                                @continue
                            @endif
                            <div class="article-block">
                                ...
                                <div class="info-row" itemprop="articleSection"><p>{!! $block_article->ua !!}</p></div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="article-blocks">
                        <div class="article-block">
                            <div class="info-row" itemprop="articleSection"><p>{!! $article->content->ua !!}</p></div>
                        </div>
                    </div>
                @endif
                        ...
                        <div class="aside-box-list">
                            @foreach($aside as $key=>$new)
                                <div class="article">
                                    ...
                                    <a href="{{route('ua.media.article', ['alias' => $new->alias, 'id' => $new->id])}}" class="article-title">{{$new->utitle}}</a>
                                </div>
                            @endforeach
                        </div>
                    </div>
