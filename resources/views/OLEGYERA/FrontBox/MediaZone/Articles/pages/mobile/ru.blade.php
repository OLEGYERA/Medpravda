@include('OLEGYERA.FrontBox.MediaZone.Articles.modules.mobile-menu.ru')
@php $is_mobile = true; @endphp

<div class="full-content media-zone">
    <section class="page-content mp-anchor" id="startPage">
        ...
        <div class="row-drug-presentation">
            <h1 class="drug-title">{{$article->title}}</h1>
            <div class="media-double-present">
                ...
                <div class="content-col">
                    <div class="navigation">
                        <h2>Cодержание</h2>
                        @include('OLEGYERA.FrontBox.MediaZone.Articles.modules.anchors.ru')
                    </div>
                </div>
            </div>
        </div>
        <div class="content-box">
            @if($article->contentRu->definition && $article->contentRu->definition !== '<p><br></p>')
                <div class="mp-row-present def" id="definition">
                    ...
                    <div class="info-row" itemprop="articleSection">{!! $article->contentRu->definition !!}</div>
                </div>
            @endif
            @include('OLEGYERA.FrontBox.MediaZone.Articles.modules.official.ru')
        </div>
        <div class="row-drug-accordion">
            <div class="accordion-item open">
                ...
                <div class="accordion-box">
                    @include('OLEGYERA.FrontBox.MODULES.editors.ru', ['data' =>  $article->dependency, 'is_mobile' => $is_mobile])
                </div>
            </div>
        </div>
    </section>
    @include('OLEGYERA.FrontBox.MODULES.asides.mobile.ru', ['data' => $article])
</div>
