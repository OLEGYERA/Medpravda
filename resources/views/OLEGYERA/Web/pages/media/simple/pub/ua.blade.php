<div class="full-content">
    ...
    <div class="mp-box pub-st">
        <div class="publication-content @if($structure_settings->fullWidth) full @endif">
            @if(!$structure_settings->bgTop)
                <div class="pub-intro">
                    <h1 itemprop="headline name" class="pub-title">{{$publication->utitle}}</h1>
                    <div class="pub-date">
                        @php $pub_dt = daysOrMonth($publication->created_at, 'ua') @endphp
                        <div class="dt-separator">Дата створення: {{$pub_dt['date']}}, {{$pub_dt['time']}}</div>
                        @php $pub_dt = daysOrMonth($publication->content->updated_at, 'ua') @endphp
                        <div class="dt-separator">Дата оновлення: {{$pub_dt['date']}}, {{$pub_dt['time']}}</div>
                    </div>
            ...
            <aside class="publication-aside">
                ...
                @foreach($aside_articles as $key=>$aside_article)
                    ...
                    @include('OLEGYERA.FrontBox.particles.article', [
                        'type' => 'mini',
                        'eclipsed' => true,
                        'link' => route('ua.pub', ['id' => $aside_article->id]),
                        'time' => daysOrMonth($aside_article->created_at, 'ua'),
                        'counter' => $aside_article->view,
                        'title' => $aside_article->utitle,
