    ...
    <div class="reactive-content flex-col">
        <div class="reactive-page-header flex-row-sb-c">
            <div class="header-left flex-col">
                <h1 class="reactive-title blue">Зависимости <i class="fas fa-angle-right"></i> ATX</h1>
                <div class="reactive-sub-titles" v-if="atxs_infoline_hidden">
                    <infoline
        ...
        <nav class="reactive-navigation">
            <router-link to="/" class="active blue">Все ATX</router-link>
        </nav>
    ...
    export default {
        ...
        methods: {
            getAtxs(search_for_alias){
                HTTP.post(`atx/get/all?page=` + this.current_paginate, {
                    filter: this.filter,
                    ...
                    this.atxs_infoline_hidden = [{title: response.data.atxs.data.length, info: 'ATX сейчас на странице'}, {title: response.data.count, info: 'всего ATX в панели управления'}]
                })
            },
            deleteAtx(id){
