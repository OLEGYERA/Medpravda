<template>
    <div class="reactive-content flex-col" v-if="parent_class_cosmetic">
        <div class="reactive-page-header flex-row-sb-c">
            <div class="header-left flex-col">
                <h1 class="reactive-title blue">Зависимости <i class="fas fa-angle-right"></i> Подклассификации Косм. Средств<i class="fas fa-angle-right"></i> {{parent_class_cosmetic.class}}</h1>
                <div class="reactive-sub-titles" v-if="class_cosmetics_infoline_hidden && pre_parent_class_cosmetic">
                    <infoline
                        :i_class="'fas fa-cubes'"
                        :visual=class_cosmetics_infoline_hidden[0].title
                        :hidden="class_cosmetics_infoline_hidden"
                    ></infoline>
                    <router-link :to="parent_class_cosmetic.parent_id != null ? {name: 'cosmetic_child', params: { alias: pre_parent_class_cosmetic.class}} : {name: 'cosmetic'}" class="r-link" style="margin-right: 15px;">Подняться на уровень выше</router-link>
                    <a href="#" class="r-link">Перейти на страницу препаратов</a>
                </div>
            </div>
            <div class="header-right">
                <div class="r-btn r-lnk" @click="callback_create = !callback_create">Добавить Подклассификацию Косм. Средств</div>
                <modal-create
                    :method_processed="'insert'"
                    :type_creation="'classes'"
                    :url="'dependency/class_cosmetic/' + parent_class_cosmetic.id"
                    :modal_title="'ATX'"
        ...
        <nav class="reactive-navigation">
            <router-link to="/" class="blue">Все Классификации Косм. Средств</router-link>
            <a class="active blue">Подклассификация Косм. Средств</a>
        </nav>
    ...
    export default {
        ...
        data: function(){
            return{
                ...
                filter_status: false,
                parent_class_cosmetic_class: null,
                parent_class_cosmetic: null,
                pre_parent_class_cosmetic: null,
            }
        },
        methods: {
            initPage(){
                this.parent_class_cosmetic_class = this.$route.params.alias;

                HTTP.get(`class_cosmetic/get/` + this.parent_class_cosmetic_class).then(response => {
                    this.parent_class_cosmetic = response.data;
                });
                HTTP.get(`class_cosmetic/get/pre-parent/` + this.parent_class_cosmetic_class).then(response => {
                    this.pre_parent_class_cosmetic = response.data;
                    console.log(this.pre_parent_class_cosmetic, 123)
                });

                this.getClassCosmetics();
            },
            getClassCosmetics(search_for_alias){
                HTTP.post(`class_cosmetic/` + this.parent_class_cosmetic_class + `/get/all?page=` + this.current_paginate, {
                    filter: this.filter,
                    ...
                    this.class_cosmetics_infoline_hidden = [{title: response.data.class_cosmetics.data.length, info: 'Подклассификаций Косм. Средств сейчас на странице'}, {title: response.data.count, info: 'всего Классификаций Косм. Средств в панеле управления'}]
                })
            },
