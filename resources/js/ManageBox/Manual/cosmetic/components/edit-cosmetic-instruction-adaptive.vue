    ...
    <div class="reactive-content flex-col">
        <div class="reactive-page-header flex-row-sb-c">
            <div class="header-left flex-col">
                <h1 class="reactive-title orange">Редактирование Косм. средства<i class="fas fa-angle-right"></i> Инструкция Адаптированная RU <i class="fas fa-angle-right"></i>{{cosmetic_alias}}</h1>
        ...
        <nav class="reactive-navigation">
            ...
            <router-link :to="{name:'edit_cosmetic_instruction_adaptive', props: {'alias': this.cosmetic_alias}}" class="active orange">Адаптированная Инструкция RU</router-link>
    ...
    export default {
        ...
        methods: {
            ...
            getInstruction(){
                ...
                this.model_status = 3;
                HTTP.get(`cosmetic/instruction/get/` + this.cosmetic_alias + `/` + 'adaptive_ru' + `/` + this.current_instruction_value).then(response => {
            ...
            updateInstructionData(data){
                this.model_status = 3;
                HTTP.post(`cosmetic/instruction/update/` + this.cosmetic_alias + `/` + 'adaptive_ru' + `/` + this.current_instruction_value, {new_data: data}).then(response => {
                    this.model_status = 4;
                })
            }
