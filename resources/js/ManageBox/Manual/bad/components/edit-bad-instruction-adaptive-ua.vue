    ...
    <div class="reactive-content flex-col">
        <div class="reactive-page-header flex-row-sb-c">
            <div class="header-left flex-col">
                <h1 class="reactive-title orange">Редактирование Бада<i class="fas fa-angle-right"></i> Инструкция Адаптированная UA <i class="fas fa-angle-right"></i>{{bad_alias}}</h1>
        ...
        <nav class="reactive-navigation">
            ...
            <router-link :to="{name:'edit_bad_instruction_adaptive_ua', props: {'alias': this.bad_alias}}" class="active orange">Адаптированная Инструкция UA</router-link>
        </nav>
    ...
    export default {
        ...
        methods: {
            ...
            getInstruction(){
                ...
                this.model_status = 3;
                HTTP.get(`bad/instruction/get/` + this.bad_alias + `/` + 'adaptive_ua' + `/` + this.current_instruction_value).then(response => {
            ...
            updateInstructionData(data){
                this.model_status = 3;
                HTTP.post(`bad/instruction/update/` + this.bad_alias + `/` + 'adaptive_ua' + `/` + this.current_instruction_value, {new_data: data}).then(response => {
                    this.model_status = 4;
                })
            }
