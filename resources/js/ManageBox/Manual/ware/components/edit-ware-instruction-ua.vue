    ...
    <div class="reactive-content flex-col">
        <div class="reactive-page-header flex-row-sb-c">
            <div class="header-left flex-col">
                <h1 class="reactive-title orange">Редактирование Мед. Изделия<i class="fas fa-angle-right"></i> Инструкция UA <i class="fas fa-angle-right"></i>{{ware_alias}}</h1>
        ...
        <nav class="reactive-navigation">
            ...
            <router-link :to="{name:'edit_ware_instruction_ua', props: {'alias': this.ware_alias}}" class="active orange">Инструкция UA</router-link>
    ...
    export default {
        ...
        methods: {
            ...
            getInstruction(){
                ...
                this.model_status = 3;
                HTTP.get(`ware/instruction/get/` + this.ware_alias + `/` + 'ua' + `/` + this.current_instruction_value).then(response => {
            ...
            updateInstructionData(data){
                this.model_status = 3;
                HTTP.post(`ware/instruction/update/` + this.ware_alias + `/` + 'ua' + `/` + this.current_instruction_value, {new_data: data}).then(response => {
                    this.model_status = 4;
                })
            }
