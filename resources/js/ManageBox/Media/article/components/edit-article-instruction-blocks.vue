<template>
    <div class="reactive-content flex-col">
        ...
        <nav class="reactive-navigation">
            ...
            <router-link :to="{name:'edit_article_instruction_blocks', props: {'alias': this.article_alias}}" class="active orange">Блочные поля RU</router-link>
            <router-link :to="{name:'edit_article_instruction_blocks_ua', props: {'alias': this.article_alias}}" class="orange">Блочные поля UA</router-link>
        </nav>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getArticleBlockList(){
                this.model_status = 3;
                HTTP.get(`article/instruction/get/blocklist/` + this.article_alias + `/` + 'ru')
                    .then(response => {
            ...
            getInstruction(){
                ...
                this.model_status = 3;
                HTTP.get(`article/instruction/get/` + this.article_alias + `/` + this.current_instruction_value + `/` + 'ru').then(response => {
            ...
            updateInstructionData(data){
                this.model_status = 3;
                HTTP.post(`article/instruction/update/` + this.article_alias + `/` + this.current_instruction_value + `/` + 'ru', {new_data: data}).then(response => {
                    this.model_status = 4;
                })
            }
