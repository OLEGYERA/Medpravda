<template>
    <div class="reactive-content flex-col">
        ...
        <nav class="reactive-navigation">
            ...
            <router-link :to="{name:'edit_article_instruction', props: {'alias': this.article_alias}}" class="orange"> Текстовое поле RU</router-link>
            <router-link :to="{name:'edit_article_instruction_ua', props: {'alias': this.article_alias}}" class="active orange">Текстовое поле UA</router-link>
        </nav>
...
export default {
    mounted() {
        ...
        this.getArticle();
        this.current_instruction_name = 'Текстовое поле Ua';
    ...
    methods: {
        ...
        getInstruction(){
            ...
            this.model_status = 3;
            HTTP.get(`article/instruction/get/` + this.article_alias + `/` + 'ua').then(response => {
        ...
        updateInstructionData(data){
            this.model_status = 3;
            HTTP.post(`article/instruction/update/` + this.article_alias + `/` + 'ua', {new_data: data}).then(response => {
                this.model_status = 4;
            })
        }
