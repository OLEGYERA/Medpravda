...
class CreateBadUaAdaptedsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('bad_ua_adapteds', function (Blueprint $table) {
            $table->increments('id');

            $table->string('title')->index();
            $table->string('slug')->unique();
            $table->foreign('slug')->references('slug')->on('bads')->onDelete('cascade')->onUpdate('cascade');
...
//      Content
    ...
    {
        Schema::dropIfExists('bad_ua_adapteds');
    }
}
