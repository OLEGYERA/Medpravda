...
class CreateWareUaAdaptedsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('ware_ua_adapteds', function (Blueprint $table) {
            $table->increments('id');
            $table->string('title')->index();
            $table->string('slug')->unique();
            $table->foreign('slug')->references('slug')->on('wares')->onDelete('cascade')->onUpdate('cascade');
...
//      Content
    ...
    {
        Schema::dropIfExists('ware_ua_adapteds');
    }
}
